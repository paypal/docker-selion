...
HUB=$(docker run -d selion/hub:1.0.0-SNAPSHOT)
...
NODE_CHROME=$(docker run -d --link $HUB_NAME:hub  selion/node-chrome$DEBUG:1.0.0-SNAPSHOT)
echo 'Starting Selenium Firefox node...'
NODE_FIREFOX=$(docker run -d --link $HUB_NAME:hub selion/node-firefox$DEBUG:1.0.0-SNAPSHOT)
